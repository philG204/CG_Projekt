             ...
             ProjectionSettings *projectionSettings)
{
  ...
  multiplyMatrices(camera->viewProj,
                   camera->projection,
                   camera->view);
...
camera_update (Camera *camera)
{
  ...
  perspective (camera->projection, camera->projectSettings.fovy,
               camera->projectSettings.aspect, camera->projectSettings.near_plane,
...
*/
void camera_setProjectionSettings(Camera *camera, ProjectionSettings *projectionSettings){
  if(memcmp(&camera->settings, projectionSettings, sizeof(ProjectionSettings)) != 0){
    memcpy(&camera->settings, projectionSettings, sizeof(ProjectionSettings));
    camera->isDirty = 1;
  }
}
