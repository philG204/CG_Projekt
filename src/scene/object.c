...
Object* object_init(char* objDir, float light[], int transparancy){
    ...
    while ((entry = readdir(dir)) != NULL) {
        ...
        const char completeTexturePath[512];
        snprintf(completeTexturePath, sizeof(completeTexturePath), "assets/%s/Textures/%s", objDir, entry->d_name);
        //printf("%s\n", completeTexturePath);
        object->material->textures[object->material->texture_count] = texture_init(completeTexturePath, object->material->shader, entry->d_name);
...
void object_transformation(Object* object, GLfloat* translation, GLfloat* scale, float rotation){
...
void object_draw(Object* object, GLfloat* cameraMatrix){
    ...
    glUniformMatrix4fv(glGetUniformLocation(object->material->shader, "viewProj"), 1, GL_FALSE, cameraMatrix);
    glUniformMatrix4fv(glGetUniformLocation(object->material->shader, "model"), 1, GL_FALSE, object->modelMatrix);

    glBindVertexArray(object->mesh->vao);

    for(int i=0;i<object->material->texture_count;i++){
        activate_texture(object->material->textures, object->material->texture_count);
    }
