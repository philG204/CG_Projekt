...
static int parseFaceToken(const char* token, FaceIndex* out)
{
    out->v = 0;
    out->vt = 0;
    out->vn = 0;
...
static int validIndex(int index, size_t count)
{
    ...
    *vertexCount = 0;
    ...
    while (fgets(line, sizeof(line), file)) {
        if (strncmp(line, "v ", 2) == 0) {
            ...
            int parts = sscanf(line, "f %127s %127s %127s %127s", t0, t1, t2, extra);

            if (parts != 3) {
                fprintf(stderr, "Ueberspringe nicht-trianguliertes Face: %s", line);
                continue;
            }

            FaceIndex idx[3];

            if (!parseFaceToken(t0, &idx[0]) ||
                !parseFaceToken(t1, &idx[1]) ||
            ...
            for (int i = 0; i < 3; ++i) {
                if (!validIndex(idx[i].v, posIndex)) {
                    fprintf(stderr, "Ungueltiger Positionsindex in Face: %s", line);
                    goto skip_face;
                }

                if (idx[i].vt != 0 && !validIndex(idx[i].vt, texIndex)) {
                    fprintf(stderr, "Ungueltiger Texturindex in Face: %s", line);
                    goto skip_face;
                }

                if (idx[i].vn != 0 && !validIndex(idx[i].vn, normIndex)) {
                    fprintf(stderr, "Ungueltiger Normalenindex in Face: %s", line);
                    goto skip_face;
                }
            }
