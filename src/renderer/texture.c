...
static Texture *
texture_get_or_load(const char *texturePath,
                    GLuint shader,
                    const char *textureName)
{
    ...
    Texture *texture = texture_init((char *)texturePath,
                                    shader,
                                    (char *)textureName);
    ...
    if (textureCacheCount >= MAX_CACHED_TEXTURES) {
        printf("texture_get_or_load: Texture-Cache voll. Texture wird ohne Cache benutzt: %s\n",
               texturePath);
        return texture;
    }

    strncpy(textureCache[textureCacheCount].path,
            texturePath,
            sizeof(textureCache[textureCacheCount].path) - 1);

    textureCache[textureCacheCount].path[
        sizeof(textureCache[textureCacheCount].path) - 1
    ] = '\0';
                         ...
                         int maxTextures)
{
    ...
    if (!config_find_line_by_key(completeConfigPath,
                                 "textures",
                                 textureLine,
                                 sizeof(textureLine))) {
    ...
    int textureCount = config_parse_string_list_value(textureLine,
                                                      textureNames,
                                                      maxTextures);
